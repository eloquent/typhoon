...
/**
 * @covers \Eloquent\Typhoon\CodeAnalysis\Issue\ParameterIssue\DocumentedParameterTypeMismatch
 ...
 */
class DocumentedParameterTypeMismatchTest extends MultiGenerationTestCase
{
    protected function setUp()
    {
        ...
        $this->_type = Phake::mock('Eloquent\Typhax\Type\Type');
        $this->_documentedType = Phake::mock('Eloquent\Typhax\Type\Type');
        $this->_severity = IssueSeverity::WARNING();
        $this->_issue = new DocumentedParameterTypeMismatch(
            $this->_classDefinition,
            $this->_methodDefinition,
            'bar',
            $this->_type,
            $this->_documentedType,
            $this->_severity
        );
    }
    ...
    public function testConstructor()
    {
        ...
        $this->assertSame($this->_type, $this->_issue->type());
        $this->assertSame($this->_documentedType, $this->_issue->documentedType());
    ...
    public function testAccept()
    {
        ...
        Phake::when($visitor)
            ->visitDocumentedParameterTypeMismatch(Phake::anyParameters())
            ->thenReturn('foo')
        ;

        $this->assertSame('foo', $this->_issue->accept($visitor));
        Phake::verify($visitor)
            ->visitDocumentedParameterTypeMismatch($this->identicalTo($this->_issue))
        ;
    }
}
